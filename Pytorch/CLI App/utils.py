...
def classify(model, image):
    ...
    temp_image = cv2.resize(src=temp_image, dsize=(SIZE, SIZE), interpolation=cv2.INTER_AREA)
...
def segment(model, image):
    h, w, _ = image.shape
    temp_image = image.copy()
    temp_image = cv2.resize(src=temp_image, dsize=(SEGMENT_SIZE, SEGMENT_SIZE), interpolation=cv2.INTER_AREA)
